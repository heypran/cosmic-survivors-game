...
// Enemy types
const ENEMY_TYPES = {
  drone: { health: 30, speed: 2.5, size: 16, color: '#ff6b6b', xp: 12, damage: 10 },
  shooter: { health: 55, speed: 1.8, size: 20, color: '#4ecdc4', xp: 25, damage: 15 },
  tank: { health: 110, speed: 1.2, size: 24, color: '#45b7d1', xp: 50, damage: 25 }
};
...
const getWeaponStats = (weapon, player) => {
  const baseWeapons = {
    laser: { damage: 18, fireRate: 250, color: '#00ff00', spread: 0, name: 'Laser' },
    plasma: { damage: 35, fireRate: 400, color: '#ff00ff', spread: 0.3, name: 'Plasma' },
    missile: { damage: 50, fireRate: 700, color: '#ffff00', spread: 0, name: 'Missile' },
    shotgun: { damage: 15, fireRate: 350, color: '#ff6600', spread: 0.4, bulletCount: 5, name: 'Shotgun' },
    lightning: { damage: 40, fireRate: 300, color: '#66ffff', spread: 0.1, name: 'Lightning' },
    railgun: { damage: 80, fireRate: 1000, color: '#ffffff', spread: 0, name: 'Railgun' },
    burst: { damage: 22, fireRate: 200, color: '#ff3399', spread: 0.15, bulletCount: 3, name: 'Burst' }
  };
...
const UPGRADE_OPTIONS = [
  { id: 'damage', name: 'Quantum Boost', description: '+25% damage', icon: '⚡' },
  { id: 'speed', name: 'Neural Enhancement', description: '+20% movement speed', icon: '🚀' },
  { id: 'health', name: 'Bio-Regenerator', description: '+25 max health', icon: '❤️' },
  { id: 'fireRate', name: 'Overclock Module', description: '+30% fire rate', icon: '🔥' },
  { id: 'plasmaWeapon', name: 'Plasma Arsenal', description: 'Unlock plasma spread shots', icon: '💥' },
  { id: 'shotgunWeapon', name: 'Scatter Gun', description: 'Unlock shotgun weapon', icon: '🎯' },
  { id: 'lightningWeapon', name: 'Arc Cannon', description: 'Unlock lightning weapon', icon: '⚡' },
  { id: 'railgunWeapon', name: 'Rail Rifle', description: 'Unlock powerful railgun', icon: '🔫' },
  { id: 'burstWeapon', name: 'Burst Fire', description: 'Unlock burst weapon', icon: '💨' },
  { id: 'multiShot', name: 'Dual Targeting', description: 'Fire at 2 nearest enemies', icon: '🎯' }
];
...
const CosmicSurvivors = () => {
  ...
  const explodeGrenade = useCallback((grenade) => {
    const particleColor = grenade.type === 'freeze' ? '#00ccff' : 
                         grenade.type === 'poison' ? '#66ff00' : 
                         grenade.type === 'nuclear' ? '#ff0066' : '#ff8800';
  ...
  const spawnEnemies = useCallback(() => {
    ...
    const actualSpawnCount = Math.min(baseSpawnCount, MAX_ENEMIES - enemies.length);
    ...
    for (let i = 0; i < actualSpawnCount; i++) {
      ...
      if (wave < 2) {
        ...
        type = rand < 0.25 ? 'drone' : (rand < 0.60 ? 'shooter' : 'tank');
      }
      ...
      switch(side) {
        case 0: x = Math.random() * GAME_WIDTH; y = -config.size; break;
        case 1: x = GAME_WIDTH + config.size; y = Math.random() * GAME_HEIGHT; break;
        case 2: x = Math.random() * GAME_WIDTH; y = GAME_HEIGHT + config.size; break;
        case 3: x = -config.size; y = Math.random() * GAME_HEIGHT; break;
  ...
  const manualShoot = useCallback(() => {
    ...
    const bulletCount = weaponStats.bulletCount || (currentWeapon === 'plasma' ? 3 : 1);
    
    const newBullets = [];
    for (let j = 0; j < bulletCount; j++) {
      const spreadAngle = bulletCount > 1 ? 
        bulletAngle + (j - (bulletCount - 1) / 2) * weaponStats.spread : bulletAngle;
      ...
      newBullets.push({
        ...
        size: currentWeapon === 'railgun' ? 6 : (currentWeapon === 'plasma' ? 4 : 3)
      });
    }
  ...
  const levelUp = useCallback(() => {
    const randomUpgrades = UPGRADE_OPTIONS
  ...
  const applyUpgrade = useCallback((upgrade) => {
    setPlayer(prev => {
      ...
      switch(upgrade.id) {
        case 'damage':
          newPlayer.damageMultiplier = Math.min(newPlayer.damageMultiplier * 1.25, 5.0);
          break;
        case 'speed':
        ...
        case 'health':
          ...
          newPlayer.health = Math.min(newPlayer.health + 25, newPlayer.maxHealth);
          break;
        case 'fireRate':
          newPlayer.fireRateMultiplier = Math.min(newPlayer.fireRateMultiplier * 1.3, 4.0);
          break;
        case 'plasmaWeapon':
  ...
  const gameLoop = useCallback(() => {
    ...
    if (deltaX !== 0 || deltaY !== 0) {
      const newX = clamp(playerPosRef.current.x + deltaX * player.speed, 16, GAME_WIDTH - 16);
      const newY = clamp(playerPosRef.current.y + deltaY * player.speed, 16, GAME_HEIGHT - 16);
    ...
    setEnemies(prevEnemies => {
      ...
      const remainingEnemies = prevEnemies.filter(enemy => {
        if (distance(enemy, playerPosRef.current) < enemy.size + 16) {
          ...
          createParticles(enemy.x, enemy.y, player.shieldActive ? '#00aaff' : '#ff0000', 5);
          return false;
        }
        return true;
      });
  ...
  }, [gameState, player, wave, grenadesSpawned, healthPacksSpawned, shieldsSpawned, grenades, healthPacks, shields, spawnEnemies, spawnGrenade, spawnHealthPack, spawnShield, explodeGrenade, createParticles, levelUp]);
  ...
  const renderGameObject = (obj, isPlayer = false) => (
    <div
      ...
      style={{
        ...
        boxShadow: isPlayer && player.shieldActive ? 
    ...
    return (
      <div className="w-full h-screen bg-gradient-to-b from-purple-900 via-blue-900 to-black flex items-center justify-center text-white">
        <div className="text-center space-y-8">
          ...
          <p className="text-xl text-gray-300 mb-8">Survive the alien onslaught on a desolate planet</p>
    ...
    return (
      <div className="w-full h-screen bg-black bg-opacity-80 flex items-center justify-center text-white fixed inset-0 z-50">
        <div className="bg-gray-900 border border-purple-500 rounded-lg p-8 max-w-md">
          <h2 className="text-2xl font-bold mb-6 text-center text-purple-400">LEVEL UP!</h2>
          <p className="text-center mb-6 text-gray-300">Choose your enhancement:</p>
            ...
            {upgrades.map(upgrade => (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="font-semibold text-purple-300">{upgrade.name}</div>
      ...
      <div className="w-full h-screen bg-gradient-to-b from-red-900 via-purple-900 to-black flex items-center justify-center text-white">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2 text-lg">
            <p>Final Score: <span className="text-purple-400 font-bold">{score}</span></p>
            <p>Waves Survived: <span className="text-blue-400 font-bold">{wave}</span></p>
            <p>Level Reached: <span className="text-green-400 font-bold">{player.level}</span></p>
            <p className="text-sm text-gray-300">
              Damage: {(player.damageMultiplier * 100).toFixed(0)}% • 
              Speed: {(player.speed / PLAYER_SPEED * 100).toFixed(0)}% • 
              Fire Rate: {(player.fireRateMultiplier * 100).toFixed(0)}%
            </p>
          </div>
          <div className="space-y-3">
            <button 
    ...
    <div className="w-full h-screen bg-black overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="fixed top-0 left-0 right-0 z-20 flex justify-between items-start p-4 bg-black bg-opacity-50 text-white">
        <div className="flex flex-col space-y-3">
          ...
          <div className="flex items-center space-x-4 text-xs">
            <div className="text-yellow-300">
              <span className="font-semibold">⚡ DMG:</span> {Math.round(player.damageMultiplier * 100)}%
            </div>
            <div className="text-blue-300">
              <span className="font-semibold">🚀 SPD:</span> {Math.round((player.speed / PLAYER_SPEED) * 100)}%
            </div>
            <div className="text-green-300">
              <span className="font-semibold">🔥 RATE:</span> {Math.round(player.fireRateMultiplier * 100)}%
            </div>
        ...
        <div className="text-right space-y-1">
          ...
          <div className="text-sm">Wave {wave} • {Math.ceil(waveTimer / 1000)}s</div>
          <div className="text-xs text-purple-300">
            Weapon: {(getWeaponStats(player.weapons[player.activeWeapon] || 'laser', player).name || 'LASER').toUpperCase()}
            {player.shieldActive && <span className="text-blue-400"> • SHIELD: {Math.ceil(player.shieldTime / 1000)}s</span>}
          </div>
        ...
        {grenades.map(grenade => (
          <div
            ...
            style={{
              ...
              border: `2px solid ${grenade.type === 'freeze' ? '#66ccff' : 
                                 grenade.type === 'poison' ? '#88ff44' : 
                                 grenade.type === 'nuclear' ? '#ff4488' : '#ffaa00'}`,
          ...
          >
            ...
            {grenade.type !== 'explosive' && (
              <div className="absolute inset-0 flex items-center justify-center text-xs font-bold text-white">
                {grenade.type === 'freeze' ? '❄️' : grenade.type === 'poison' ? '☢️' : '☢️'}
              </div>
            )}
          </div>
        ))}
      ...
      <div className="mt-4 text-gray-400 text-sm text-center">
        ...
        {wave >= 10 && <span> • Blue shields provide temporary protection</span>}
      </div>
    </div>
  );
};
